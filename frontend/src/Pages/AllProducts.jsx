...
const AllProducts = () => {
    ...
    // Sync category/sub from URL params on mount
    useEffect(() => {
        const catParam = searchParams.get('category');
        const subParam = searchParams.get('sub');
        if (catParam) setSelectedCategories([catParam]);
        if (subParam) setSelectedSubs([subParam]);
    }, []); // eslint-disable-line react-hooks/exhaustive-deps

    const brands = useMemo(() => {
        const b = new Set();
        all_product.forEach(p => { if (p.brand) b.add(p.brand); });
        return Array.from(b).sort();
    }, [all_product]);

    const availableSubs = useMemo(() => {
        const s = new Set();
        all_product.forEach(p => { if (p.subcategory) s.add(p.subcategory); });
    ...
    const filteredProducts = useMemo(() => {
        ...
        return result;
    }, [all_product, selectedCategories, selectedSubs, selectedPrice, selectedBrand, sortBy]);

    const hasActiveFilters = selectedCategories.length > 0 || selectedSubs.length > 0 || selectedPrice !== 0 || selectedBrand !== '';
                    ...
                    {hasActiveFilters && (
                        <div className="sc-active-filters">
                            {selectedCategories.map(c => {
                                ...
                                return (
                                    <span key={c} className="filter-tag" onClick={() => toggleCategory(c)}>
                                        {cat ? cat.label : c} ×
                                    </span>
                                );
                            })}
